            ...
            <Stack>
                {actionData?.state !== AppSetupState.CreateUser &&
                    <Paper bg="red.1" p="lg" m="sm" radius="md">
                        ...
                        <Text my="md">
                            Please contact a librarian to help resolve this issue.
                        </Text>
                    </Paper>
                }
                <Paper withBorder shadow="md" p="lg" m="sm" radius="md">
                    <Text size="sm"><strong>Admins:</strong> Please enter one-time code found in server console to enter setup menu:</Text>
                    <PinInput
                        mt="sm"
                    ...
                    {actionData?.state == AppSetupState.CreateUser &&
                        <>
                            <Text my="md">
                                <b>Welcome to your library server!</b> We&apos;re showing this setup screen
...
export async function action({ request }: ActionFunctionArgs): Promise<TypedResponse<{ state: AppSetupState, errors?: CreateUserErrors }>> {
    ...
    if (typeof otp == "string" && checkOtp(otp, setupOtpHash)) {
        if (typeof formData.get("username") == "string") { // If the user might be on the second stage
            ...
            if (hasErrors) {
                return json({ state: AppSetupState.CreateUser, errors: errors });
            }
            return redirect(`/`);
        }
